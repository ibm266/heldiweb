...
- [ ] **Apply `supabase/migrations/0004_review_media_limits.sql` by hand in the SQL Editor.** Re-confirmed live 4 Aug 2026: the bucket still returns `file_size_limit: null, allowed_mime_types: null`. Worth being precise about why this is not merely a quota concern. With `allowed_mime_types` unset, a holder of a signed upload URL can PUT any content type, including HTML, and Supabase will serve it back with that type from a `supabase.co` origin. The size limit is the quota half; the mime allow-list is the half that stops the bucket being used to host someone else's content. Measured today: `review-media` returns `{"file_size_limit":null,"allowed_mime_types":null}`. `docs/security.md:58-60` calls this "the only check in the upload path that cannot be bypassed" and both `lib/rate-limit.ts:38-42` and `app/api/reviews/upload-url/route.ts:44-51` defer to it, so the documentation asserts a control that is not there. Anyone holding a signed URL can PUT any size and any content type. The Supabase project is shared with another app, so filling its quota is not only Heldi's problem. Never use `supabase db push`. **(Mihir, dashboard)**
