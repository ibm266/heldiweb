...
- [ ] Nav: `/shop/chai` is reachable from the sitemap and from Our story, and it links out to `/shop`, but nothing on `/shop` links back to it and there is no "Shop" submenu. Add at least a link from the Khana page. Two buyable products probably means a `/shop` listing page or a nav split (PLAYBOOK.md has recipes for both, and neither is free: `SubpageNav` keeps duplicated mobile and desktop link lists that must change together)
