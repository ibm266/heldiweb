...
- [~] Nav: `/shop/chai` is now reachable from the homepage range band (`components/range-section.tsx`, "Meet Chai" beside "Meet Khana" / "Shop Khana"), from the Khana page description ("Meet Heldi Chai", 2 Sep 2026), from Our story and from the sitemap, and the homepage FAQ answers "Is there a Heldi for chai?". Still open: the nav's single "Shop" link goes to Khana only. Two buyable products probably means a `/shop` listing page or a nav split (PLAYBOOK.md has recipes for both, and neither is free: `SubpageNav` keeps duplicated mobile and desktop link lists that must change together)
