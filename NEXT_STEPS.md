...
- [x] **Is Chai in the next run at all, and is Dahi ahead of it?** **Yes, Chai is in run 1. Decided 4 Sep 2026.** That un-parks it, and the Gate 2 line in HeldiPM (`context/product-gates.md:17`, "Chai spices not ordered this run") and the two-SKU Khana + Dahi launch in `data/project-status.json` are both now out of date: the spices have to be ordered. The method-of-addition question that `data/project-status.json:829` parked it over is answered by the site, which teaches the cold-milk BREW / COOL / STIR method on `/shop/chai` rather than reformulating. Record both in HeldiPM. Earlier note kept for the record: HeldiPM's newest decision log (`context/product-gates.md`) still carries the Gate-2 line *"Chai spices not ordered this run"* and the Gate-3 line *"Chai market potentially larger, parked for behaviour not demand"*, and `data/project-status.json` names a two-SKU launch of **Khana and Dahi**, with Chai deferred. The August work resumed Chai development (testing day 16 Aug, print round 13 on 19 Aug, COGS 25 Aug) but no gate entry un-parks it, and the website has never heard of Dahi. The page as shipped does not depend on this answer. Every item below does
